        ...
        private void ExportXML()
        {
            ...
            using (XmlWriter writer = XmlWriter.Create(saveXML.FileName, settings))
            {
                ...
                foreach (Message message in m_Messages)
                {
                    ...
                    writer.WriteAttributeString("index", (message.index).ToString());
                    writer.WriteAttributeString("id", String.Format("{0:X4}", message.index));
                    writer.WriteString(message.msgData);
