    ...
    public partial class LevelEditorForm : Form
    {
        ...
        private void glLevelView_MouseWheel(object sender, MouseEventArgs e)
        {
            if ((m_MouseDown == MouseButtons.Left) && ((m_Selected >> 28) != 0xF) && (m_LastClicked == m_Selected))
            {
                ...
                /*
                Vector3 vec = Vector3.Multiply(Vector3.Normalize(Vector3.Subtract(m_CamPosition, m_SelectedObject.Position)),delta);
                m_SelObjTotalMov.X += vec.X;
                m_SelObjTotalMov.Y += vec.Y;
                m_SelObjTotalMov.Z += vec.Z;
                */
